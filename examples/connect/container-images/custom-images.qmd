...
With `backends.kubernetes`, use `executionEnvironments` instead of `launcher.customRuntimeYaml` to define content images. Unlike `customRuntimeYaml`, changes to `executionEnvironments` take effect on every `helm upgrade` without requiring a pod restart or database reset.
