...
The `PersistentVolume` allows setting NFS `mountOptions` and it creates a storage class that Posit Workbench then takes advantage of when it creates its `PersistentVolumeClaim`.
