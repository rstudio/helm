...
- In root mode (`pod.runAsRoot: true`), the pod now sets `fsGroup` (and `fsGroupChangePolicy: OnRootMismatch`) to `serviceAccountUserId` when `launcher.enabled` is true. The Kubernetes launcher runs as the non-root `serviceAccountUser` even in a root pod; without a group-readable ServiceAccount token it failed to start (`Permission denied` reading the token). Non-launcher root deployments are unaffected.
- In rootless mode (`pod.runAsRoot: false`), the chart-managed `secure-cookie-key` is now projected into `/mnt/secret-configmap/rstudio/` even when it is autogenerated. The rootless `launcher.conf` default points `secure-cookie-key-file` there so the launcher shares rserver's cookie key; previously the file was unmounted with the default key and the launcher read a missing file.
