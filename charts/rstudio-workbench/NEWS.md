...
- **Rootless Workbench pods.** New `pod.runAsRoot` value (default `true`); set it to `false` to run the Workbench pod as a non-root user. In rootless mode user management uses SCIM provisioning (the bundled SSSD daemon is now controlled by the new `config.sssd` block; `config.userProvisioning` is deprecated), and launcher sessions default to nss mode — they start directly as the requesting user and resolve identity in-session via the pwb NSS module, so the session image must ship `libnss_pwb` and the `pwb` `nsswitch.conf` entries.
