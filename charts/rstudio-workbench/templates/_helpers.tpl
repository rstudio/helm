...
- name: rstudio
  ...
  volumeMounts:
    ...
    {{- /* Also mount in rootless mode: the rootless launcher.conf default points
           secure-cookie-key-file at /mnt/secret-configmap/rstudio/secure-cookie-key, which is
           projected from the chart-managed secret even when the key is autogenerated. */ -}}
    {{- if or .Values.launcherPem.existingSecret .Values.launcherPem.value .Values.secureCookieKey.existingSecret .Values.secureCookieKey.value .Values.global.secureCookieKey.existingSecret .Values.global.secureCookieKey.value .Values.config.database.conf.existingSecret .Values.config.secret .Values.config.existingSecrets (not .Values.pod.runAsRoot) }}
    - name: rstudio-secret
...
{{- if .Values.global.secureCookieKey.existingSecret }}
    - secret:
        ...
        - key: secure-cookie-key
          ...
          mode: {{ $secretMode }}
{{- end }}
{{- /* Project secure-cookie-key from the chart-managed secret when not using an existingSecret.
       Fires for an explicit .value, and also for the autogenerated key in rootless mode, where the
       launcher.conf default reads the shared key from /mnt/secret-configmap/rstudio/secure-cookie-key
       (so the launcher does not self-generate a key that fails to verify rserver-signed requests). */ -}}
{{- if and (or .Values.secureCookieKey.value .Values.global.secureCookieKey.value (not .Values.pod.runAsRoot)) (not (or .Values.secureCookieKey.existingSecret .Values.global.secureCookieKey.existingSecret)) }}
    - secret:
        name: {{ include "rstudio-workbench.fullname" . }}-secret
        items:
        - key: secure-cookie-key
