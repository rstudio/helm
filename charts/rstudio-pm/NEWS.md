...
- To handle the migration of existing data owned by `root`, there is now a Helm
  hook that essentially runs chown on the data directory every time a user runs
  `helm upgrade`. Unfortunately, we can't detect when we actually need to run
  this migration, so it currently runs unconditionally. The hook can be disabled
  by setting `enableMigrations=false`; in the future when we no longer expect
  users to have root-owned data, this will become the default.
